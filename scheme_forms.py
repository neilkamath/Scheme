...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(signature):
        ...
        key = signature.first
        rv = expressions.rest
        definition = do_lambda_form(Pair(formals, rv), env)
        env.bindings[key] = definition
        return key # return signature.first
        # END PROBLEM 10
    else:
...
def do_and_form(expressions, env):
    ...
    if len(expressions) == 0:
        return True
    result = True
    while nil != expressions:
        if expressions.rest is nil:
            result = scheme_eval(expressions.first, env, True)
        else:
            result = scheme_eval(expressions.first, env, False)
        if False is result:
            return False
...
def make_let_frame(bindings, env):
    ...
    # BEGIN PROBLEM 14
    while nil != bindings:
        branches = bindings.first
        validate_form(branches, 2, 2)
        
        names = Pair(branches.first, names)
        vals = Pair(scheme_eval(branches.rest.first, env), vals)
